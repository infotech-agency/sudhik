...
const getHighlightIcon = (text: string) => {
  ...
  //     {
  //       id: product._id,
  //       title: selectedVariant ? `${product.title} — ${selectedVariant.label}` : product.title,
  //       price: activePrice,
  //       image: images[0],
  //     },
  //     qty,
              ...
              {product.variants!.map((v) => {
                ...
                return (
                  <button
                    ...
                    `}
                  >
                    <p className="font-serif text-sm text-ink/90">{v.label}</p>
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {product.benefits
                ...
                .slice(0, 6)
                .map((h) => (
                  <span key={h} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gold-50 border border-gold-400/25 font-sans text-sm font-medium text-ink/80">
                    <Sparkles size={11} className="text-gold-400" />
                    {h}
                  </span>
                ))}

...
export default function ProductInfoTabs({ product }: { product: Product }) {
  ...
  return (
    <div className="mt-10 sm:mt-16">
      ...
      <div className="flex gap-1 sm:gap-2 border-b border-gold-400/20 overflow-x-auto scrollbar-none -mx-5 px-5 sm:mx-0 sm:px-0">
        {tabs.map((t) => (
          <button
            key={t}
            onClick={() => setActive(t)}
            className={`relative shrink-0 px-3.5 sm:px-6 py-3 sm:py-3.5 font-royal text-[11px] sm:text-sm tracking-royal-sm uppercase whitespace-nowrap transition-colors ${
              active === t ? 'text-saffron-500' : 'text-ink/50 hover:text-ink'
            }`}
          >
            {t}
      ...
      <div className="pt-6 sm:pt-10 min-h-0">
        <AnimatePresence mode="wait">
          <motion.div
            key={active}
            initial={{ opacity: 0, y: 14 }}
          ...
          >
            ...
            {active === 'Overview' && (
              <div className="grid lg:grid-cols-2 gap-6 sm:gap-10 items-center">
                <div>
                  <p className="font-deva text-lg sm:text-2xl text-saffron-600/80 mb-2 sm:mb-4">
                    शुद्धि
                  </p>
                  <h3 className="font-display text-2xl sm:text-4xl font-bold text-ink mb-3 sm:mb-5">
                    {product.title}
                  </h3>
                  <p className="font-serif text-base sm:text-xl font-medium text-ink/90 leading-relaxed">
                    {product.description}
                  </p>
                </div>
                <div className="rounded-2xl border border-gold-400/20 bg-gradient-to-br from-gold-50/40 to-sand/30 p-5 sm:p-8">
                  <p className="font-royal text-xs tracking-royal uppercase text-saffron-500 mb-3 sm:mb-4">
                    At a Glance
                  </p>
                  <dl className="space-y-3">
                    {[
                      ...
                      <div key={k} className="flex justify-between gap-4 py-2 border-b border-gold-400/10">
                        <dt className="font-royal text-[11px] sm:text-xs tracking-royal uppercase text-ink/60">
                          {k}
                        </dt>
                        <dd className="font-serif text-sm sm:text-base font-medium text-ink/90 text-right">
                          {v}
                        </dd>
                      </div>
                    ))}
              ...
              <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                {splitList(product.benefits).map((b, i) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: i * 0.08 }}
                    className="flex gap-3 sm:gap-4 p-4 sm:p-6 rounded-2xl bg-white border border-gold-400/15"
                  >
                    <span className="shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-br from-saffron-500 to-maroon-500 text-ivory font-royal text-xs sm:text-sm flex items-center justify-center">
                      {i + 1}
                    </span>
                    <p className="font-serif text-sm sm:text-xl font-medium text-ink/90 pt-1">
                      {b}
                    </p>
                  </motion.div>
                ))}
            ...
            {/* ---------- INGREDIENTS ---------- */}
            {active === 'Ingredients' && (
              <p className="font-serif text-sm sm:text-xl font-medium text-ink/90 leading-relaxed max-w-3xl">
                {product.ingredients || 'Ingredient details coming soon.'}
              </p>
            )}
              ...
              <dl className="grid sm:grid-cols-2 gap-x-10 gap-y-2">
                {[
                  ...
                  <div key={k} className="flex justify-between gap-4 py-3 border-b border-gold-400/10">
                    ...
                    <dd className="font-serif text-sm sm:text-lg font-medium text-ink/90 text-right max-w-[60%]">
                      {v}
                    </dd>
                  </div>
                ))}

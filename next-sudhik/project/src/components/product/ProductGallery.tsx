...
//           {activeImg.src && (
//             <>
//               <button onClick={() => setFullscreen(true)} aria-label="View fullscreen" className="absolute top-4 right-4 p-2.5 rounded-full bg-ivory/70 backdrop-blur-sm border border-gold-400/30 text-ink hover:bg-gold-400 hover:text-ink transition-all opacity-0 group-hover:opacity-100">
//                 <Maximize2 size={16} />
//               </button>
//               <button onClick={prev} aria-label="Previous image" className="absolute left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-ivory/70 backdrop-blur-sm border border-gold-400/30 text-ink hover:bg-maroon-500 hover:text-ivory transition-all opacity-0 group-hover:opacity-100">
//                 <ChevronLeft size={18} />
//               </button>
//               <button onClick={next} aria-label="Next image" className="absolute right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-ivory/70 backdrop-blur-sm border border-gold-400/30 text-ink hover:bg-maroon-500 hover:text-ivory transition-all opacity-0 group-hover:opacity-100">
...
//           <div className="hidden lg:block absolute -right-[440px] top-0 w-[400px] h-[400px] rounded-2xl overflow-hidden border border-gold-400/40 shadow-premium pointer-events-none bg-ivory" aria-hidden>
...
//           Hover to zoom · {activeImg.label} · {active + 1} / {images.length}
//         </p>
//       </div>
...
//             animate={{ opacity: 1 }}
//             exit={{ opacity: 0 }}
//             className="fixed inset-0 z-[80] bg-ink/90 backdrop-blur-md flex items-center justify-center p-6"
//             onClick={() => setFullscreen(false)}
//           >
//             <button className="absolute top-6 right-6 p-3 rounded-full bg-ivory/10 text-ivory hover:bg-gold-400 hover:text-ink transition-all" aria-label="Close fullscreen">
//               <X size={22} />
//             </button>
//             <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-4 sm:left-8 p-3 rounded-full bg-ivory/10 text-ivory hover:bg-gold-400 hover:text-ink transition-all" aria-label="Previous">
//               <ChevronLeft size={24} />
//             </button>
//             <motion.img key={active} src={activeImg.src} alt={activeImg.alt} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-[85vw] max-h-[82vh] rounded-2xl object-contain border border-gold-400/30" onClick={(e) => e.stopPropagation()} draggable={false} />
//             <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-4 sm:right-8 p-3 rounded-full bg-ivory/10 text-ivory hover:bg-gold-400 hover:text-ink transition-all" aria-label="Next">
//               <ChevronRight size={24} />
//             </button>
//             <span className="absolute bottom-6 left-1/2 -translate-x-1/2 font-royal text-xs tracking-royal uppercase text-ivory/60">

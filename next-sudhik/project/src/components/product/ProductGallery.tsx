...
export default function ProductGallery({ product }: { product: Product }) {
  ...
  return (
    <div className="flex flex-col-reverse gap-4 lg:gap-6 w-full min-w-0">
      {/* min-w-0 = ye scroll strip ab apni content-width se page ko wide nahi karegi */}
      {/* thumbnails ab hamesha niche hi rahenge — sm:flex-row/sm:flex-col hata diya taaki desktop pe left side na jaayein */}
      <div className="flex gap-3 overflow-x-auto no-scrollbar min-w-0 shrink-0">
        {images.map((img, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
